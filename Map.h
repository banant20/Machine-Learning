...
public:
  ...
  Iterator find(const Key_type& k) const {
      Pair_type key;
      key.first = k;
      key.second = Value_type();

      return the_tree.find(key);
  }
  ...
  std::pair<Iterator, bool> insert(const Pair_type& val) {
      if (the_tree.end() == the_tree.find(val)) {
          the_tree.insert(val);
      }
      std::pair<Iterator, bool> x = std::make_pair(the_tree.find(val), false);
      return x;
  }
...
private:
    BinarySearchTree<Pair_type,PairComp> the_tree;
