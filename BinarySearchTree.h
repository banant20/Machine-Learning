  ...
  //          No iteration or recursion is allowed.
    ...
    static int height_impl(const Node* node) {
        if (node == nullptr) {
            ...
            return std::max(height_impl(node->right), height_impl(node->left)) + 1;
        }
    }
    ...
    static Node* find_impl(Node* node, const T& query, Compare less) {
        ...
        //if query is less than datum at a node, go to left child
        else if (less(query, node->datum)) {
            return find_impl(node->left, query, less);
        }
        //if query is greater than datum at a node, go to right child
        else {
    ...
    static bool check_sorting_invariant_impl(const Node* node, Compare less) {
        ...
        if (node->right != nullptr) {
            if (!less(node->datum, max_element_impl(node->right)->datum)) {
                return false;
            }
        ...
        if (node->left != nullptr) {
            ...
            if (!less(min_element_impl(node->left)->datum, node->datum)) {
                return false;
            }
        }
